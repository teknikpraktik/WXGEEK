...
import { SUNRISE_ALT, solarElevation, sunEvents, sunPath } from "./sun";
...
test("Karlstad 26 september 2026: soluppgång 06:59, solnedgång 18:54, middagshöjd 29–30°", () => {
  ...
  assert.deepEqual(ev.map((e) => e.kind), ["sunrise", "sunset"]);
  assert.ok(near(ev[0].t, "06:59"), `soluppgång ${local(ev[0].t)}`);
  assert.ok(near(ev[1].t, "18:54"), `solnedgång ${local(ev[1].t)}`);
...
test("soluppgång och solnedgång ligger där solhöjden passerar −0,833° (med refraktion)", () => {
  const day = Date.UTC(2026, 8, 25, 22);
  for (const e of sunEvents(...KSD, day, day + 24 * H)) {
    assert.ok(Math.abs(solarElevation(...KSD, e.t) - SUNRISE_ALT) < 0.01);
  }
  ...
  assert.ok(near(malmo[0]?.t, "04:24") && near(malmo[1]?.t, "21:55"));
});

test("polarfall: ingen uppgång eller nedgång, men banan finns", () => {
  ...
  assert.deepEqual(sunEvents(67.855, 20.225, winter, winter + 24 * H), [], "polarnatt i Kiruna");
